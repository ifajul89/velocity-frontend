        ...
        <div className="p-4 sm:p-6 grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          <Card>
            ...
            <CardContent className="space-y-2">
              <div>
                <Label>Name</Label>
                <div className="text-sm">{order.customerFirstName || 'N/A'} {order.customerLastName || ''}</div>
              </div>
              <div>
                <Label>Email</Label>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <div>
                ...
                <div className="text-sm">{format(new Date(order.createdAt), "MMM d, yyyy, h:mm a")}</div>
              </div>
              <div>
                <Label>Status</Label>
            ...
            <Card className="md:col-span-2">
              ...
              <CardContent>
                <div className="space-y-4">
                  {order.trackingUpdates.map((update: TrackingUpdate, index: number) => (
            ...
            <Card className="md:col-span-2">
              ...
              <CardContent>
                <div className="flex justify-between">
                  {Object.entries(order.trackingStages).map(([stage, completed]: [string, boolean]) => (
...
export default function OrdersManagementPage() {
  ...
  const applySearch = (e: React.FormEvent) => {
    ...
    const filtered = orders.filter((order) => {
      ...
      return (
        ...
        (order.customerFirstName && order.customerFirstName.toLowerCase().includes(searchLower)) ||
  ...
  const handleEdit = (
    ...
    if (!messageToUse && orderToEdit.trackingUpdates && orderToEdit.trackingUpdates.length > 0) {
        messageToUse = orderToEdit.trackingUpdates[orderToEdit.trackingUpdates.length - 1].message || "";
    }
  ...
  const handleSave = async (orderId: string) => {
    ...
    try {
        ...
        const updatedOrder: ApiOrder = {
            ...originalOrder,
            status: tempData.status,
            estimatedDelivery: tempData.estimatedDelivery || originalOrder.estimatedDelivery,
            trackingStages: {
                                ...
                                <TableCell colSpan={8}>
                                  <div className="flex flex-col space-y-4 py-4">
                                    ...
                                    <div className="space-y-2">
                                      ...
                                      />
                                      <div className="text-xs text-gray-500">
                                        A message is required for the status update
                                      </div>
                                    </div>
                                    <div className="flex justify-end gap-2">
                                      ...
                                      <Button
                                        onClick={() => {
                                            console.log('Save button clicked. Current tempData:', tempData);
                                      ...
                                      <DropdownMenuItem 
                                        onClick={() =>
                                          handleEdit(
                                            order._id,
                                            order.status,
                                            order.estimatedDelivery || order.estimatedDeliveryDate || "",
                                            order.trackingUpdates && order.trackingUpdates.length > 0 
                                              ? order.trackingUpdates[order.trackingUpdates.length - 1].message 
                                              : ""
                                          )
                                        }
                                        className="text-xs cursor-pointer"

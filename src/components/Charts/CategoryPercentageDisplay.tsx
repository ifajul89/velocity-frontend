    ...
    return categoryData.map((entry, index) => {
      ...
      return (
        <div key={`legend-${index}`} className="flex items-center">
          ...
          />
          <span className="text-xs text-gray-700">{entry.name} {percent.toFixed(0)}%</span>
        </div>
      );
    });
  };
  ...
  if (isLoading) {
    return (
      <div className="w-full h-full flex items-center justify-center">
        <div className="h-12 w-12 rounded-full border-2 border-indigo-200 border-t-indigo-600 animate-spin"></div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
      <div className="flex items-center">
        ...
        <div className="ml-auto text-xs font-medium px-2 py-1 bg-indigo-50 text-indigo-600 rounded-full">By Category</div>
      </div>
